...
public class PrimeNumberFinder {
	...
	public PrimeNumberFinder(String filePath) {
	...
	public ArrayList<Long> findPrimesUnder(int numberToLookTo) {
			...
			if (counter != 5) {
				if (isPrime(number)) {
					...
					if ((System.nanoTime() - start1) > 8_333_333) {
						label.setText(String.format("%.3f %%", 100 * (number - startingNumb)
								/ (double) (percentMax)));
		...
		System.out.printf("Found %,d prime numbers between 2 and %,d\n", this.primeNumbers.size(),
				numberToLookTo);
		...
		frame.dispose();
		System.out.println("primes = " + this.primeNumbers);
		return primeNumbers;
	}
	...
	private double loadPrimeList(int numberToLoadTo) {
